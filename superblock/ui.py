...
def run_ui() -> None:
    ...
    class App:
        def __init__(self, root: tk.Tk) -> None:
            ...
            self.metrics_var = tk.StringVar(value="day=0 | score=0.000 | mse=0.000000 | visible_hits=0")
        ...
        def draw_trust_graph(self) -> None:
            ...
            if not values:
                self.trust_canvas.create_text(180, 90, text="暂无数据", fill="#333")
                return

            max_v = max(values)
            min_v = min(values)
            if max_v == min_v:
                y = h - 14 if max_v == 0 else h // 2
                self.trust_canvas.create_line(10, y, w - 10, y, fill="#000", width=2)
                msg = f"全部为常数: {int(max_v)}"
                if max_v == 0:
                    msg = "全部为0：当前可视单元格未被命中"
                self.trust_canvas.create_text(180, 14, text=msg, fill="#333")
                return
        ...
        def run_training(self, cfg: UIConfig) -> None:
            ...
            for day_idx in range(1, cfg.days + 1):
                ...
                self.metrics_var.set(
                    f"day={day_idx} | score={score:.3f} | mse={pos_mse:.6f} | visible_hits={visible_hits}"
                )
